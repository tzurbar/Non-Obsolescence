---
title: How to Replace a Light Bulb
productName: Standard Light Fixture
category: Home & Electrical
difficulty: beginner
estimatedTime: 5 minutes
tools:
  - Replacement bulb
  - Step stool (if needed)
coverImage: /guides/lightbulb/cover.svg
featured: true
authorName: Tzur
publishDate: 2026-08-23
partLinks:
  - label: "LED bulb, E26 base, warm white (example retailer)"
    url: "https://example.com/parts/led-bulb-e26"
videoLinks:
  - label: "Full walkthrough (2 min)"
    url: "https://www.youtube.com/watch?v=example-placeholder"
steps:
  - text: "Turn off the power. Switch off the fixture or unplug the lamp."
    image: /guides/lightbulb/step-1.svg
  - text: "Remove the old bulb. Turn it counter-clockwise (for standard screw bases) until it comes free. For bayonet-mount bulbs, push in slightly and twist."
    image: /guides/lightbulb/step-2.svg
    videoLinks:
      - label: "Bayonet-mount removal, close-up (30s)"
        url: "https://www.youtube.com/watch?v=example-placeholder"
  - text: "Check the socket. Make sure it's clean and the contacts aren't corroded or bent."
  - text: "Insert the new bulb. Turn it clockwise until snug — don't overtighten."
    image: /guides/lightbulb/step-4.svg
    partLinks:
      - label: "Same bulb model used in this guide"
        url: "https://example.com/parts/led-bulb-e26"
  - text: "Restore power and test. Flip the switch back on."
---

## Safety first

Turn off the light switch, and if you're working on a lamp, unplug it. Let the old bulb cool down before touching it if it's been on recently.

## Disposal note

Incandescent and LED bulbs can usually go in household trash, but CFLs contain a small amount of mercury and should be taken to a hazardous-waste or recycling drop-off point.
